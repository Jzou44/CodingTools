---
name: coding-tools-json-minifier
description: "Use when Codex needs to run the Coding.Tools JSON Minifier MCP tool (json-minifier) through tools/list and tools/call for Minify JSON by removing whitespace and formatting."
---

# JSON Minifier

Use the Coding.Tools MCP tool `json-minifier` when the user needs: Minify JSON by removing whitespace and formatting.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `json-minifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `json-minifier`.
...
5. Call `tools/call` with `params.name = "json-minifier"`.
...
Web UI: https://coding.tools/json-minifier.html
