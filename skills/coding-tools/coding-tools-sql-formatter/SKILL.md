---
name: coding-tools-sql-formatter
description: "Use when Codex needs to run the Coding.Tools SQL Formatter MCP tool (sql-formatter) through tools/list and tools/call for Format and beautify SQL queries with proper indentation."
---

# SQL Formatter

Use the Coding.Tools MCP tool `sql-formatter` when the user needs: Format and beautify SQL queries with proper indentation.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `sql-formatter`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `sql-formatter`.
...
5. Call `tools/call` with `params.name = "sql-formatter"`.
...
Web UI: https://coding.tools/sql-formatter.html
