---
name: coding-tools-compress-png
description: "Use when Codex needs to run the Coding.Tools Compress PNG MCP tool (compress-png) through tools/list and tools/call for Reduce PNG file size by converting to palette mode with optimization."
---

# Compress PNG

Use the Coding.Tools MCP tool `compress-png` when the user needs: Reduce PNG file size by converting to palette mode with optimization.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `compress-png`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `compress-png`.
...
5. Call `tools/call` with `params.name = "compress-png"`.
...
{
  "type": "object",
  "description": "Compress PNG returns isError=true with a resource_link to the browser UI because server-side image processing is not available."
}
```
...
This is a browser-only image tool in MCP. `tools/call` returns `isError: true` with a `resource_link` to https://coding.tools/compress-png.html instead of processing image bytes server-side.

Web UI: https://coding.tools/compress-png.html
