---
name: coding-tools-css-minifier
description: "Use when Codex needs to run the Coding.Tools CSS Minifier MCP tool (css-minifier) through tools/list and tools/call for Minify CSS by removing whitespace, comments, and unnecessary characters."
---

# CSS Minifier

Use the Coding.Tools MCP tool `css-minifier` when the user needs: Minify CSS by removing whitespace, comments, and unnecessary characters.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `css-minifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `css-minifier`.
...
5. Call `tools/call` with `params.name = "css-minifier"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Minified CSS string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/css-minifier.html
