---
name: coding-tools-xml-minifier
description: "Use when Codex needs to run the Coding.Tools XML Minifier MCP tool (xml-minifier) through tools/list and tools/call for Minify XML by removing whitespace and formatting."
---

# XML Minifier

Use the Coding.Tools MCP tool `xml-minifier` when the user needs: Minify XML by removing whitespace and formatting.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `xml-minifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `xml-minifier`.
...
5. Call `tools/call` with `params.name = "xml-minifier"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Minified XML string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/xml-minifier.html
