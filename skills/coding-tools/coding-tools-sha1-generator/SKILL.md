---
name: coding-tools-sha1-generator
description: "Use when Codex needs to run the Coding.Tools SHA1 Generator MCP tool (sha1-generator) through tools/list and tools/call for Generate SHA-1 hash from any text input."
---

# SHA1 Generator

Use the Coding.Tools MCP tool `sha1-generator` when the user needs: Generate SHA-1 hash from any text input.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `sha1-generator`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `sha1-generator`.
...
5. Call `tools/call` with `params.name = "sha1-generator"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "SHA-1 hash as lowercase hexadecimal."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/sha1-generator.html
