---
name: coding-tools-json-formatter
description: "Use when Codex needs to run the Coding.Tools JSON Formatter MCP tool (json-formatter) through tools/list and tools/call for Format and beautify JSON code with proper indentation."
---

# JSON Formatter

Use the Coding.Tools MCP tool `json-formatter` when the user needs: Format and beautify JSON code with proper indentation.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `json-formatter`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `json-formatter`.
...
5. Call `tools/call` with `params.name = "json-formatter"`.
...
Web UI: https://coding.tools/json-formatter.html
