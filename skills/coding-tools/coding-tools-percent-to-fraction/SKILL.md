---
name: coding-tools-percent-to-fraction
description: "Use when Codex needs to run the Coding.Tools Percent to Fraction MCP tool (percent-to-fraction) through tools/list and tools/call for Convert percentages to fractions."
---

# Percent to Fraction

Use the Coding.Tools MCP tool `percent-to-fraction` when the user needs: Convert percentages to fractions.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `percent-to-fraction`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `percent-to-fraction`.
...
5. Call `tools/call` with `params.name = "percent-to-fraction"`.
...
Web UI: https://coding.tools/percent-to-fraction.html
