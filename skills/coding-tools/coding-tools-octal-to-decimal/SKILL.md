---
name: coding-tools-octal-to-decimal
description: "Use when Codex needs to run the Coding.Tools Octal to Decimal MCP tool (octal-to-decimal) through tools/list and tools/call for Convert octal numbers to decimal format."
---

# Octal to Decimal

Use the Coding.Tools MCP tool `octal-to-decimal` when the user needs: Convert octal numbers to decimal format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `octal-to-decimal`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `octal-to-decimal`.
...
5. Call `tools/call` with `params.name = "octal-to-decimal"`.
...
Web UI: https://coding.tools/octal-to-decimal.html
