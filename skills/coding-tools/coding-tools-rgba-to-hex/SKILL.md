---
name: coding-tools-rgba-to-hex
description: "Use when Codex needs to run the Coding.Tools RGBA to Hex MCP tool (rgba-to-hex) through tools/list and tools/call for Convert RGBA color values to hex color codes with opacity."
---

# RGBA to Hex

Use the Coding.Tools MCP tool `rgba-to-hex` when the user needs: Convert RGBA color values to hex color codes with opacity.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `rgba-to-hex`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `rgba-to-hex`.
...
5. Call `tools/call` with `params.name = "rgba-to-hex"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Hex color string in #RRGGBBAA format."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/rgba-to-hex.html
