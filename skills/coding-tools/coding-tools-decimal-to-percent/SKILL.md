---
name: coding-tools-decimal-to-percent
description: "Use when Codex needs to run the Coding.Tools Decimal to Percent MCP tool (decimal-to-percent) through tools/list and tools/call for Convert decimal numbers to percentages."
---

# Decimal to Percent

Use the Coding.Tools MCP tool `decimal-to-percent` when the user needs: Convert decimal numbers to percentages.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `decimal-to-percent`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `decimal-to-percent`.
...
5. Call `tools/call` with `params.name = "decimal-to-percent"`.
...
Web UI: https://coding.tools/decimal-to-percent.html
