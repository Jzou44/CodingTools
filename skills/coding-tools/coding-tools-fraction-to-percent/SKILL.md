---
name: coding-tools-fraction-to-percent
description: "Use when Codex needs to run the Coding.Tools Fraction to Percent MCP tool (fraction-to-percent) through tools/list and tools/call for Convert fractions to percentages."
---

# Fraction to Percent

Use the Coding.Tools MCP tool `fraction-to-percent` when the user needs: Convert fractions to percentages.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `fraction-to-percent`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `fraction-to-percent`.
...
5. Call `tools/call` with `params.name = "fraction-to-percent"`.
...
Web UI: https://coding.tools/fraction-to-percent.html
