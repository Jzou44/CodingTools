---
name: coding-tools-binary-to-decimal
description: "Use when Codex needs to run the Coding.Tools Binary to Decimal MCP tool (binary-to-decimal) through tools/list and tools/call for Convert binary numbers to decimal format."
---

# Binary to Decimal

Use the Coding.Tools MCP tool `binary-to-decimal` when the user needs: Convert binary numbers to decimal format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `binary-to-decimal`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `binary-to-decimal`.
...
5. Call `tools/call` with `params.name = "binary-to-decimal"`.
...
Web UI: https://coding.tools/binary-to-decimal.html
