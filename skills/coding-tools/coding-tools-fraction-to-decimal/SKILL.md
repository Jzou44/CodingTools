---
name: coding-tools-fraction-to-decimal
description: "Use when Codex needs to run the Coding.Tools Fraction to Decimal MCP tool (fraction-to-decimal) through tools/list and tools/call for Convert fractions to decimal numbers."
---

# Fraction to Decimal

Use the Coding.Tools MCP tool `fraction-to-decimal` when the user needs: Convert fractions to decimal numbers.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `fraction-to-decimal`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `fraction-to-decimal`.
...
5. Call `tools/call` with `params.name = "fraction-to-decimal"`.
...
Web UI: https://coding.tools/fraction-to-decimal.html
