---
name: coding-tools-regex-tester
description: "Use when Codex needs to run the Coding.Tools Regex Tester MCP tool (regex-tester) through tools/list and tools/call for Test regular expressions live with match highlighting and flag controls."
---

# Regex Tester

Use the Coding.Tools MCP tool `regex-tester` when the user needs: Test regular expressions live with match highlighting and flag controls.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `regex-tester`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `regex-tester`.
...
5. Call `tools/call` with `params.name = "regex-tester"`.
...
Web UI: https://coding.tools/regex-tester.html
