---
name: coding-tools-decimal-to-hex
description: "Use when Codex needs to run the Coding.Tools Decimal to Hex MCP tool (decimal-to-hex) through tools/list and tools/call for Convert decimal numbers to hexadecimal format."
---

# Decimal to Hex

Use the Coding.Tools MCP tool `decimal-to-hex` when the user needs: Convert decimal numbers to hexadecimal format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `decimal-to-hex`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `decimal-to-hex`.
...
5. Call `tools/call` with `params.name = "decimal-to-hex"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Uppercase hexadecimal integer string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/decimal-to-hex.html
