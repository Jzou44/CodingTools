---
name: coding-tools-number-to-words
description: "Use when Codex needs to run the Coding.Tools Number to Words MCP tool (number-to-words) through tools/list and tools/call for Convert numbers to English words, ordinals, and ordinal numbers."
---

# Number to Words

Use the Coding.Tools MCP tool `number-to-words` when the user needs: Convert numbers to English words, ordinals, and ordinal numbers.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `number-to-words`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `number-to-words`.
...
5. Call `tools/call` with `params.name = "number-to-words"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Number rendered as English words or ordinals."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/number-to-words.html
