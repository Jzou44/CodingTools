---
name: coding-tools-decimal-to-octal
description: "Use when Codex needs to run the Coding.Tools Decimal to Octal MCP tool (decimal-to-octal) through tools/list and tools/call for Convert decimal numbers to octal format."
---

# Decimal to Octal

Use the Coding.Tools MCP tool `decimal-to-octal` when the user needs: Convert decimal numbers to octal format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `decimal-to-octal`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `decimal-to-octal`.
...
5. Call `tools/call` with `params.name = "decimal-to-octal"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Octal integer string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/decimal-to-octal.html
