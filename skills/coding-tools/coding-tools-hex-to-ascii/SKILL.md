---
name: coding-tools-hex-to-ascii
description: "Use when Codex needs to run the Coding.Tools Hex to ASCII MCP tool (hex-to-ascii) through tools/list and tools/call for Convert hexadecimal values to ASCII text."
---

# Hex to ASCII

Use the Coding.Tools MCP tool `hex-to-ascii` when the user needs: Convert hexadecimal values to ASCII text.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `hex-to-ascii`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `hex-to-ascii`.
...
5. Call `tools/call` with `params.name = "hex-to-ascii"`.
...
Web UI: https://coding.tools/hex-to-ascii.html
