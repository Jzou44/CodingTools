---
name: coding-tools-hex-to-binary
description: "Use when Codex needs to run the Coding.Tools Hex to Binary MCP tool (hex-to-binary) through tools/list and tools/call for Convert hexadecimal numbers to binary format."
---

# Hex to Binary

Use the Coding.Tools MCP tool `hex-to-binary` when the user needs: Convert hexadecimal numbers to binary format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `hex-to-binary`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `hex-to-binary`.
...
5. Call `tools/call` with `params.name = "hex-to-binary"`.
...
Web UI: https://coding.tools/hex-to-binary.html
