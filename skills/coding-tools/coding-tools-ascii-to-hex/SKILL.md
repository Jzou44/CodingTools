---
name: coding-tools-ascii-to-hex
description: "Use when Codex needs to run the Coding.Tools ASCII to Hex MCP tool (ascii-to-hex) through tools/list and tools/call for Convert ASCII text to hexadecimal values."
---

# ASCII to Hex

Use the Coding.Tools MCP tool `ascii-to-hex` when the user needs: Convert ASCII text to hexadecimal values.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `ascii-to-hex`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `ascii-to-hex`.
...
5. Call `tools/call` with `params.name = "ascii-to-hex"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Hexadecimal byte pairs separated by spaces."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/ascii-to-hex.html
