---
name: coding-tools-roman-numerals-to-numbers
description: "Use when Codex needs to run the Coding.Tools Roman to Numbers MCP tool (roman-numerals-to-numbers) through tools/list and tools/call for Convert Roman numerals to decimal numbers."
---

# Roman to Numbers

Use the Coding.Tools MCP tool `roman-numerals-to-numbers` when the user needs: Convert Roman numerals to decimal numbers.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `roman-numerals-to-numbers`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `roman-numerals-to-numbers`.
...
5. Call `tools/call` with `params.name = "roman-numerals-to-numbers"`.
...
```json
{
  ...
  "method": "tools/call",
  "params": {
    "name": "roman-numerals-to-numbers",
    "arguments": {
        "input": "MMXXVI"
    }
  }
...
Web UI: https://coding.tools/roman-numerals-to-numbers.html
