---
name: coding-tools-sha256-generator
description: "Use when Codex needs to run the Coding.Tools SHA256 Generator MCP tool (sha256-generator) through tools/list and tools/call for Generate SHA-256 hash from any text input."
---

# SHA256 Generator

Use the Coding.Tools MCP tool `sha256-generator` when the user needs: Generate SHA-256 hash from any text input.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `sha256-generator`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `sha256-generator`.
...
5. Call `tools/call` with `params.name = "sha256-generator"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "SHA-256 hash as lowercase hexadecimal."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/sha256-generator.html
