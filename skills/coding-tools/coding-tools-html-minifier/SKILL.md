---
name: coding-tools-html-minifier
description: "Use when Codex needs to run the Coding.Tools HTML Minifier MCP tool (html-minifier) through tools/list and tools/call for Minify HTML by removing whitespace, comments, and unnecessary characters."
---

# HTML Minifier

Use the Coding.Tools MCP tool `html-minifier` when the user needs: Minify HTML by removing whitespace, comments, and unnecessary characters.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `html-minifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `html-minifier`.
...
5. Call `tools/call` with `params.name = "html-minifier"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Minified HTML string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/html-minifier.html
