---
name: coding-tools-sql-minifier
description: "Use when Codex needs to run the Coding.Tools SQL Minifier MCP tool (sql-minifier) through tools/list and tools/call for Minify SQL by removing whitespace, comments, and unnecessary formatting."
---

# SQL Minifier

Use the Coding.Tools MCP tool `sql-minifier` when the user needs: Minify SQL by removing whitespace, comments, and unnecessary formatting.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `sql-minifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `sql-minifier`.
...
5. Call `tools/call` with `params.name = "sql-minifier"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Minified SQL string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/sql-minifier.html
