---
name: coding-tools-md5-generator
description: "Use when Codex needs to run the Coding.Tools MD5 Generator MCP tool (md5-generator) through tools/list and tools/call for Generate MD5 hash from any text input."
---

# MD5 Generator

Use the Coding.Tools MCP tool `md5-generator` when the user needs: Generate MD5 hash from any text input.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `md5-generator`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `md5-generator`.
...
5. Call `tools/call` with `params.name = "md5-generator"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "MD5 hash as lowercase hexadecimal."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/md5-generator.html
