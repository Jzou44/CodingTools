---
name: coding-tools-progressive-jpeg
description: "Use when Codex needs to run the Coding.Tools Progressive JPEG MCP tool (progressive-jpeg) through tools/list and tools/call for Convert baseline JPEG images to progressive JPEG format."
---

# Progressive JPEG

Use the Coding.Tools MCP tool `progressive-jpeg` when the user needs: Convert baseline JPEG images to progressive JPEG format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `progressive-jpeg`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `progressive-jpeg`.
...
5. Call `tools/call` with `params.name = "progressive-jpeg"`.
...
{
  "type": "object",
  "description": "Progressive JPEG returns isError=true with a resource_link to the browser UI because server-side image processing is not available."
}
```
...
This is a browser-only image tool in MCP. `tools/call` returns `isError: true` with a `resource_link` to https://coding.tools/progressive-jpeg.html instead of processing image bytes server-side.

Web UI: https://coding.tools/progressive-jpeg.html
