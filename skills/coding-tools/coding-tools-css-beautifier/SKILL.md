---
name: coding-tools-css-beautifier
description: "Use when Codex needs to run the Coding.Tools CSS Beautifier MCP tool (css-beautifier) through tools/list and tools/call for Beautify and format CSS code with proper indentation."
---

# CSS Beautifier

Use the Coding.Tools MCP tool `css-beautifier` when the user needs: Beautify and format CSS code with proper indentation.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `css-beautifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `css-beautifier`.
...
5. Call `tools/call` with `params.name = "css-beautifier"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Formatted CSS string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/css-beautifier.html
