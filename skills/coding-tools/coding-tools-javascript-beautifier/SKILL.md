---
name: coding-tools-javascript-beautifier
description: "Use when Codex needs to run the Coding.Tools JavaScript Beautifier MCP tool (javascript-beautifier) through tools/list and tools/call for Beautify and format JavaScript code with proper indentation."
---

# JavaScript Beautifier

Use the Coding.Tools MCP tool `javascript-beautifier` when the user needs: Beautify and format JavaScript code with proper indentation.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `javascript-beautifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `javascript-beautifier`.
...
5. Call `tools/call` with `params.name = "javascript-beautifier"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Formatted JavaScript string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/javascript-beautifier.html
