---
name: coding-tools-xml-formatter
description: "Use when Codex needs to run the Coding.Tools XML Formatter MCP tool (xml-formatter) through tools/list and tools/call for Format and beautify XML code with proper indentation."
---

# XML Formatter

Use the Coding.Tools MCP tool `xml-formatter` when the user needs: Format and beautify XML code with proper indentation.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `xml-formatter`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `xml-formatter`.
...
5. Call `tools/call` with `params.name = "xml-formatter"`.
...
Web UI: https://coding.tools/xml-formatter.html
