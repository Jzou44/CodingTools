---
name: coding-tools-photo2pixel
description: "Use when Codex needs to run the Coding.Tools Photo2Pixel MCP tool (photo2pixel) through tools/list and tools/call for Convert photos to 8-bit pixel art with adjustable style and pixel size."
---

# Photo2Pixel

Use the Coding.Tools MCP tool `photo2pixel` when the user needs: Convert photos to 8-bit pixel art with adjustable style and pixel size.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `photo2pixel`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `photo2pixel`.
...
5. Call `tools/call` with `params.name = "photo2pixel"`.
...
{
  "type": "object",
  "description": "Photo2Pixel returns isError=true with a resource_link to the browser UI because server-side image processing is not available."
}
```
...
This is a browser-only image tool in MCP. `tools/call` returns `isError: true` with a `resource_link` to https://coding.tools/photo2pixel.html instead of processing image bytes server-side.

Web UI: https://coding.tools/photo2pixel.html
