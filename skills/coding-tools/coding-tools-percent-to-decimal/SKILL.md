---
name: coding-tools-percent-to-decimal
description: "Use when Codex needs to run the Coding.Tools Percent to Decimal MCP tool (percent-to-decimal) through tools/list and tools/call for Convert percentages to decimal numbers."
---

# Percent to Decimal

Use the Coding.Tools MCP tool `percent-to-decimal` when the user needs: Convert percentages to decimal numbers.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `percent-to-decimal`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `percent-to-decimal`.
...
5. Call `tools/call` with `params.name = "percent-to-decimal"`.
...
Web UI: https://coding.tools/percent-to-decimal.html
