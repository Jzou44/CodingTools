---
name: coding-tools-sha384-generator
description: "Use when Codex needs to run the Coding.Tools SHA384 Generator MCP tool (sha384-generator) through tools/list and tools/call for Generate SHA-384 hash from any text input."
---

# SHA384 Generator

Use the Coding.Tools MCP tool `sha384-generator` when the user needs: Generate SHA-384 hash from any text input.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `sha384-generator`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `sha384-generator`.
...
5. Call `tools/call` with `params.name = "sha384-generator"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "SHA-384 hash as lowercase hexadecimal."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/sha384-generator.html
