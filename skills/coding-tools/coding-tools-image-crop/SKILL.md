---
name: coding-tools-image-crop
description: "Use when Codex needs to run the Coding.Tools Image Crop MCP tool (image-crop) through tools/list and tools/call for Crop PNG, JPEG, and WebP images with a draggable crop box."
---

# Image Crop

Use the Coding.Tools MCP tool `image-crop` when the user needs: Crop PNG, JPEG, and WebP images with a draggable crop box.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `image-crop`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `image-crop`.
...
5. Call `tools/call` with `params.name = "image-crop"`.
...
{
  "type": "object",
  "description": "Image Crop returns isError=true with a resource_link to the browser UI because server-side image processing is not available."
}
```
...
This is a browser-only image tool in MCP. `tools/call` returns `isError: true` with a `resource_link` to https://coding.tools/image-crop.html instead of processing image bytes server-side.

Web UI: https://coding.tools/image-crop.html
