---
name: coding-tools-html-beautifier
description: "Use when Codex needs to run the Coding.Tools HTML Beautifier MCP tool (html-beautifier) through tools/list and tools/call for Beautify and format HTML code with proper indentation."
---

# HTML Beautifier

Use the Coding.Tools MCP tool `html-beautifier` when the user needs: Beautify and format HTML code with proper indentation.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `html-beautifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `html-beautifier`.
...
5. Call `tools/call` with `params.name = "html-beautifier"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Formatted HTML string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/html-beautifier.html
