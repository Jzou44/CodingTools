---
name: coding-tools-reverse-text
description: "Use when Codex needs to run the Coding.Tools Reverse Text MCP tool (reverse-text) through tools/list and tools/call for Reverse any text string instantly."
---

# Reverse Text

Use the Coding.Tools MCP tool `reverse-text` when the user needs: Reverse any text string instantly.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `reverse-text`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `reverse-text`.
...
5. Call `tools/call` with `params.name = "reverse-text"`.
...
Web UI: https://coding.tools/reverse-text.html
