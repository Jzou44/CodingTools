---
name: coding-tools-binary-to-hex
description: "Use when Codex needs to run the Coding.Tools Binary to Hex MCP tool (binary-to-hex) through tools/list and tools/call for Convert binary numbers to hexadecimal format."
---

# Binary to Hex

Use the Coding.Tools MCP tool `binary-to-hex` when the user needs: Convert binary numbers to hexadecimal format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `binary-to-hex`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `binary-to-hex`.
...
5. Call `tools/call` with `params.name = "binary-to-hex"`.
...
Web UI: https://coding.tools/binary-to-hex.html
