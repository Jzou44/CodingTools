---
name: coding-tools-javascript-minifier
description: "Use when Codex needs to run the Coding.Tools JavaScript Minifier MCP tool (javascript-minifier) through tools/list and tools/call for Minify JavaScript by removing whitespace, comments, and shortening variable names."
---

# JavaScript Minifier

Use the Coding.Tools MCP tool `javascript-minifier` when the user needs: Minify JavaScript by removing whitespace, comments, and shortening variable names.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `javascript-minifier`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `javascript-minifier`.
...
5. Call `tools/call` with `params.name = "javascript-minifier"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Minified JavaScript string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/javascript-minifier.html
