---
name: coding-tools-decimal-to-binary
description: "Use when Codex needs to run the Coding.Tools Decimal to Binary MCP tool (decimal-to-binary) through tools/list and tools/call for Convert decimal numbers to binary format."
---

# Decimal to Binary

Use the Coding.Tools MCP tool `decimal-to-binary` when the user needs: Convert decimal numbers to binary format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `decimal-to-binary`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `decimal-to-binary`.
...
5. Call `tools/call` with `params.name = "decimal-to-binary"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Binary integer string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/decimal-to-binary.html
