---
name: coding-tools-decimal-to-fraction
description: "Use when Codex needs to run the Coding.Tools Decimal to Fraction MCP tool (decimal-to-fraction) through tools/list and tools/call for Convert decimal numbers to fractions."
---

# Decimal to Fraction

Use the Coding.Tools MCP tool `decimal-to-fraction` when the user needs: Convert decimal numbers to fractions.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `decimal-to-fraction`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `decimal-to-fraction`.
...
5. Call `tools/call` with `params.name = "decimal-to-fraction"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Reduced fraction string."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/decimal-to-fraction.html
