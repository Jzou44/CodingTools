---
name: coding-tools-url-decode
description: "Use when Codex needs to run the Coding.Tools URL Decode / Query Parser MCP tool (url-decode) through tools/list and tools/call for Decode URL-encoded text and parse query-string parameters."
---

# URL Decode / Query Parser

Use the Coding.Tools MCP tool `url-decode` when the user needs: Decode URL-encoded text and parse query-string parameters.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `url-decode`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `url-decode`.
...
5. Call `tools/call` with `params.name = "url-decode"`.
...
```json
{
  ...
  "properties": {
    "result": {
      "type": "string",
      "description": "Decoded URL text and optional query parameter listing."
    }
  },
  "required": [
    "result"
...
Web UI: https://coding.tools/url-decode.html
