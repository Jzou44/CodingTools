---
name: coding-tools-exif-viewer
description: "Use when Codex needs to run the Coding.Tools EXIF Viewer MCP tool (exif-viewer) through tools/list and tools/call for View EXIF metadata embedded in photos including GPS, camera, and date info."
---

# EXIF Viewer

Use the Coding.Tools MCP tool `exif-viewer` when the user needs: View EXIF metadata embedded in photos including GPS, camera, and date info.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `exif-viewer`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `exif-viewer`.
...
5. Call `tools/call` with `params.name = "exif-viewer"`.
...
{
  "type": "object",
  "description": "EXIF Viewer returns isError=true with a resource_link to the browser UI because server-side image processing is not available."
}
```
...
This is a browser-only image tool in MCP. `tools/call` returns `isError: true` with a `resource_link` to https://coding.tools/exif-viewer.html instead of processing image bytes server-side.

Web UI: https://coding.tools/exif-viewer.html
