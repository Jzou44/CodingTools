---
name: coding-tools-json-to-xml
description: "Use when Codex needs to run the Coding.Tools JSON to XML MCP tool (json-to-xml) through tools/list and tools/call for Convert JSON data to XML format."
---

# JSON to XML

Use the Coding.Tools MCP tool `json-to-xml` when the user needs: Convert JSON data to XML format.
...
When an MCP client hides JSON-RPC details, configure the endpoint, select tool `json-to-xml`, and pass only the Example Arguments object as the tool arguments.
...
2. Call `tools/list`.
3. Find the tool whose `name` is `json-to-xml`.
...
5. Call `tools/call` with `params.name = "json-to-xml"`.
...
Web UI: https://coding.tools/json-to-xml.html
